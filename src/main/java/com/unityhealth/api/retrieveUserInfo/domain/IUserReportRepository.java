...
public interface IUserReportRepository extends JpaRepository<Tblaccount, Integer>{
   ...
   // Tblaccount findByVUsernameOrByVEmailAndVStatus(String vUserName,String vEmail,String vStatus);
    ...
    List<Tblaccount> findByVFirstNameAndVLastNameAndStore_storeNameAndStore_Group_IdInAndVStatus(String vFirstName, String vLastName,String storeName,List<Integer> groupIDs,String vStatus);
...
" (concat(coalesce(trim(account.store.vLocAddress1),''),coalesce(trim(account.store.vLocAddress2),'')) = ?3  AND trim(account.store.vLocLocality1)=?4"
    ...
    @Query("SELECT account FROM Tblaccount account WHERE account.apiSyncStatus=?1 and account.iBreezeUserID is null " )
    List<Tblaccount> findByApiSyncStatus(String apiSyncStatus);
}
