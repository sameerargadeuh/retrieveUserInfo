...
" o.concat(coalesce(trim(account.store.vLocAddress1),''),coalesce(trim(account.store.vLocAddress2),'')) = ?3  AND trim(account.store.vLocLocality1)=?4"
             + "AND trim(account.store.storeRegion)=?5 AND trim(account.store.postCode)=?6 and account.vFirstName=?1 and account.vLastName=?2 ")
    Tblaccount findByAddress();
}
